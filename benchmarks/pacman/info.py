...
def readCommand( argv ):
    ...
    parser.add_option('--replay-from-states', dest='statesToReplay',
                      help='A recorded game file (sequence of states) to replay', default=None)
    ...
    if options.statesToReplay != None :
        print 'Replaying recorded game %s.' % options.gameToReplay
        recorded = {}
        recorded['layout'] = args['layout']
        recorded['states_file'] = options.statesToReplay
        recorded['display'] = args['display']
        replayGameFromStates(**recorded)
        sys.exit(0)
...
def load_states( layout, filename ) :
    ...
    with open(filename) as input :
        for line in input :
            if 's=State' not in line : continue
            line = line.split('[')
