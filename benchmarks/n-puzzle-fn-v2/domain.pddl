...
;;; n-puzzle Domain
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; The standard n-puzzle problem, functional version without nested fluents.
...
(define (domain fn-n-puzzle)
    ...
    (:action swap
    :parameters (?p - position ?blank_pos - position)
    :precondition (and
        (= (blank) ?blank_pos)
        (adjacent ?p ?blank_pos)
    )
    :effect (and
        (assign (blank) ?p)
        (assign (tile_at ?blank_pos) (tile_at ?p))
        (assign (tile_at ?p) 0))
    )
