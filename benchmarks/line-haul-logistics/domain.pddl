...
(define (domain fn_line_haul)
	...
	(:functions
		...
		(day_cost ) - cost
		(dist ?l1 - location ?l2 - location) - distance ;; distance in kms between two locations
	...
	(:action move_truck
		:parameters	(?t - truck ?l2 - location)
		:precondition	(and
					(not (= (location ?t) ?l2) )
				)
		:effect		(and
					(assign (location ?t) ?l2)
					(assign (day_cost) (+ (day_cost) (* (op_cost ?t) (dist (location ?t) ?l2))))
				)
	)
)
